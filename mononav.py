...
This script is designed for an ArduCopter drone that is configured correctly as described in the ArduCopter setup instructions file and 
may require modification for your specific hardware!
...
STREAM_URL = config['camera_src']                                 # ESP32 HTTP MJPEG stream
save_during_flight = config['save_during_flight']
enable_undistort = config['enable_undistort']
traj_period = config['traj_period']
colormap = config.get('depth_map_enable', True)                   # for depth visualization
...
last_chosen_traj = None
last_action_state = None                                           # Track printing state for repeated modes so we only print once per mode transition.
...
fallback_spin_rate = config.get('fallback_spin_rate', 0.3)
...
if traj_period != "None":                                             # Overide period. Smaller values than that defined in generate_primitives may be beneficial
    period = traj_period
...
print("Press 'g' to enable MonoNav autonomous mode, or a/w/d for manual left/straight/right, e/q for manual yaw", flush=True)
...
def main():
    ...
    while True:
        ...
        print("Connected.")
        mavc.en_pose_stream(25)                                       # Commands AP to stream poses at given frequency. Keep this higher than the pose thread, else latency increases
        mavc.set_ekf_origin(EKF_LAT, EKF_LON, 0)                      # Ignored if already close to the previous origin, if set
                                                                      # Setting an EKF origin is not necessary on ArduCopter 4.7 atleast
    ...
    # This accounts for camera resolution mismatch, separate from crop/undistort transformations
    if calib_width is not None and calib_height is not None:
        ...
        if not (abs(frame_width - calib_width) < 1e-6 and abs(frame_height - calib_height) < 1e-6):
            print(f"[camera] Resolution scaled; new focal length: fx={mtx[0,0]:.1f}, fy={mtx[1,1]:.1f} pixels")

    # Then apply crop adjustment (if undistort enabled) or use resolution-adjusted matrix
    # for VBG initialization - must respect enable_undistort flag to match image transform
    if enable_undistort and optimal_mtx is not None and roi is not None:
        vbg_intrinsics = get_cropped_intrinsics(optimal_mtx, roi)
    elif enable_undistort and optimal_mtx is not None:
        vbg_intrinsics = optimal_mtx
    else:
        vbg_intrinsics = mtx if mtx is not None else get_ideal_intrinsics(frame_width, frame_height)
    ...
    try:
        ...
        while True: # Run until VBG is populated to prevent planning error. This is checked by choose_primitive returning without exception
            ...
            print("TIME TO COMPUTE DEPTH (s):", time.perf_counter() - start_time_test)
        ...
        while not shouldStop:
            mode = mavc.get_mode()
            if mode == 'LAND':
                break                                                      # exit control loop if we are landing using external command, to prevent sending velocity commands that may interfere with landing
            if max_traj_idx is None:
                ...
                if no_traj_count > 3:                                        # if we fail to find a trajectory for 3 consecutive iterations, it may be safer to stop the vehicle and wait for human intervention rather than continue trying to execute unsafe trajectories
                    ...
                    if last_chosen_traj is not None:
                        ...
                        yaw_rate = fallback_spin_rate
                    print("Yawing. Direction:", yaw_direction, flush=True)
                    yaw_vehicle(yaw_rate)
                else:
                    print("[INFO] No safe trajectory found. Hovering in place.", flush=True)

            else:
                no_traj_count = 0                                            # reset counter after taking evasive action
            ...
            # emergency stop key gets priority (these exit the control loop). Important that they break out of the loop
            ...
            if (last_key_pressed != 'g' and last_key_pressed != 'f'):
            ...
            while time.perf_counter() - start_time <= period:
                ...
                # compute depth
                depth_numpy, depth_colormap = compute_depth(transform_bgr, depth_anything, INPUT_SIZE, make_colormap=colormap)
                camera_position = get_pose_matrix(*pose)
                ...
                # TODO: This can probably be put outside the control loop while decreasign weight_threshold of vbg
                if save_during_flight:
                    ...
                    np.save(transform_depth_dir + '/' + 'transform_frame-%06d.depth.npy'%(frame_number), depth_numpy)  # saved in millimeters
            ...
            if shouldStop:                                                  # exit if shouldStop
                mavc.set_mode('LAND')
                break
