...
def _heartbeat_loop(interval_s=1.0):
    """
    Internal loop that sends a GCS heartbeat at regular intervals.
    """
    global _hb_stop_event
    while not _hb_stop_event.is_set():
        try:
            send_heartbeat()
        except Exception:
            # Swallow errors; connection handling is done elsewhere
            pass
        _hb_stop_event.wait(interval_s)

def start_heartbeat_thread(freq_hz=1.0):
    """
    Start a background thread that sends heartbeats at `freq_hz`.
    Calling multiple times is safe (will not start multiple threads).
    """
    global _hb_thread, _hb_stop_event
    if _hb_thread is not None and _hb_thread.is_alive():
        return
    _hb_stop_event = threading.Event()
    _hb_thread = threading.Thread(target=_heartbeat_loop, args=(1.0/max(1e-6, freq_hz),), daemon=True)
    _hb_thread.start()
...
def connect_drone(IP, baud=115200):
    ...
    while not drone.wait_heartbeat(timeout=2):
        send_heartbeat()
    # Start background heartbeat sender at 1 Hz to keep UDP telemetry alive
    try:
        start_heartbeat_thread(freq_hz=1.0)
        printd("Heartbeat sender thread started (1 Hz)")
    except Exception:
        printd("Failed to start heartbeat thread")
